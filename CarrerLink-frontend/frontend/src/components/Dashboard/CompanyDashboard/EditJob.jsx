...
import { updateJob } from "../../../api/JobDetailsApi";

function EditJob({ job, onClose, onSubmit, companyId }) {
    ...
    // In EditJob.jsx - Update state initialization
    useEffect(() => {
        if (job) {
            setJobData({
                jobId: job.jobId,
                ...
                technologies: job.technologies?.length ?
                    job.technologies.map(t => ({
                        techId: t.techId, // Preserve existing IDs
                        techName: t.techName
                    })) :
                    [{ techName: "" }],
                status: job.status?.toUpperCase() || "ACTIVE" // Ensure uppercase for enum
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.success) {
                onSubmit({
                    ...jobData,
                    // Add any transformed fields if needed
                });
